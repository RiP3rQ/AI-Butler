...
import { getFiles } from "@/components/providers/queries/getFilesQuery";
...
export type appFoldersType = any & { files: File[] | [] };
export type appWorkspacesType = any & {
          ...
          if (workspace.id === action.payload.workspaceId) {
            return {
              ...
              folders: workspace.folders.filter(
                (folder: any) => folder.id !== action.payload.folderId,
              ),
            };
          }
          return workspace;
      ...
      return {
        ...
        workspaces: state.workspaces.map((workspace) => {
          if (workspace.id === action.payload.workspaceId) {
            return {
              ...workspace,
              folder: workspace.folders.map((folder: any) => {
                if (folder.id === action.payload.folderId) {
                  return {
                    ...folder,
                    files: folder.files.filter(
                      (file: any) => file.id !== action.payload.fileId,
                    ),
                  };
                }
                return folder;
