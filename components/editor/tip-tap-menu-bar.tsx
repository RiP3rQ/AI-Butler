...
const TipTapMenuBar = ({ editor, isPending }: Props) => {
  return (
    <div className={"flex w-full items-center justify-between"}>
      ...
      {isPending ? (
        <SavingBadge text={"Saving..."} color={"yellow"} />
      ) : (
        <SavingBadge text={"Saved"} color={"green"} />
      )}
    </div>
  );
};
...
const SavingBadge = ({ text, color }: { text: string; color: string }) => {
  return <Badge className={`bg-${color}-500 h-6 text-white`}>{text}</Badge>;
};
